...
\begin{minipage}{\linewidth}
    ...
    \captionof{figure}[Volume Visualization]{Languages speed relative to C (averaged benchmark results), plotted against the length of the needed code. The source can be found on Github\cite{Benchmarks}\cite{JuliaBench}.}
...
If the previous demands are taken into account, the options shrink down considerably.
...
At this point, there are close to zero libraries left. As pointed out in Figure \ref{fig:juliabench}, Matlab, Python, and R disqualify, as they are too slow. JavaScript, Java, Go, and Lua are missing a scientific background and the others are too low level for the described goals.
...
The lowest relevant abstraction is the programming language.
...
Based on this abstraction, GLVisualize\cite{GLVisualize} was implemented, which renders the \ac{OpenGL} accelerated visualizations.
...
This way, a single function, e.g. \textit{visualize}, can create a default visualization for every data type. 
Instead of manually passing additional information to the visualization function most information is already encoded into the type.
...
This makes it possible to visualize variables without further user interaction, which is perfect for visual debugging. 
For a more fine-grained control of the visualization, more information is needed.

Addition information can be passed via key word arguments. An optional argument to the function is a style type, which controls the chosen defaults and the type of the visualization.
...
Most influential language constructs are its hierarchical type system and multiple dispatch.
...
One can build type hierarchies like this:
\begin{lstlisting}
abstract Number
...
What will happen at runtime is, that Julia compiles a method specialized on the arguments which result in overloading the function with a concretely typed method.
...
Like this, if the function does not access nonconstant global values, all types inside the function will be known at call time.
...
This is perfect for integrating low-level libraries like \ac{OpenGL} and the \ac{OpenCL}.
...
In this thesis, Julia's own benchmark suite is used in addition to some real world benchmarks.
...
\begin{minipage}{\linewidth}
    ...
    \label{fig:juliabench2}
\end{minipage}

In the first benchmark from Figure \ref{fig:juliabench2}, we can see that Julia stays well within the range of C speed. 
...
The results\cite{JuliaFSCpp} have been, that F\# comes out last with 748.275 ms, then Julia with 483.769 ms and finally C++ with 463.474 ms. 
...
It is comparing finite element solvers, which is an often used algorithm in material research and, therefore, represents a relevant use case for Julia.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l|c|c|c}
        ...
        \hline
    \end{tabular}
    \captionof{table}[FEM Benchmark]{Performance of an FEM solver written in Julia compared to some existing libraries. \cite{FMSolver}}
...
\ac{LLVM} is a compiler infrastructure, which has front ends for different languages and compiles to different platforms like x86, ARM, \ac{OpenCL} (via SPIR) and the \ac{CUDA} (via NVPTX). 
A language designer must create an \ac{AST} which \ac{LLVM} can convert into \ac{LLVM}'s \ac{IR}. This \ac{IR} forms a standardized basis for any optimization step. Every language that can be converted to \ac{LLVM} \ac{IR} can be combined at this level. \ac{LLVM} itself offers many optimizations, but also third party optimization can be integrated.
This yields superior language interfacing for \ac{LLVM} based languages, as inlining and other optimizations can be done across the boundary of one language.
...
\ac{LLVM} is also used by Clang\cite{Clang}, the C/C++ front end for \ac{LLVM} rivaling \ac{gcc}, it is used by Apple's programming language Swift\cite{SWIFT} and for Mozilla's language Rust\cite{Rust}.
...
If C++ code that is compiled with \ac{gcc} is much faster than the same code compiled with \ac{LLVM}, it can be expected that the \ac{gcc} version will also be faster than a comparable Julia program.
...
In Figure \ref{fig:opengl} the basic architecture of an OpenGL program pipeline is shown.
...
In order to have a working OpenGL rendering pipeline at least a vertex shader and a fragment shader needs to be written.
...
Data gets loaded into the shaders via buffers, textures, and uniforms. Buffers are 1D arrays, textures 1D/2D/3D arrays with both having their own memory while uniforms live in the program object.
...
Displaying objects can only be achieved by rasterizing pixels to the framebuffer via the fragment shader.
The framebuffer can be sent directly to the monitor.
Framebuffers can contain multiple render targets, which are the buffers the fragment shader can write to.
The write operation is heavily restricted. The fragment shader can only write to the location calculated by the vertex shader and simultaneously reads from the framebuffer are not possible. 
...
Each signal has a rank, in order to build up a sorted heap with this information.
...
This can be used for creating advanced contexts that share the memory with another context.
...
As shown in Figure \ref{fig:architecture}, Romeo uses GLVisualize for creating \ac{GUI} elements and the visualizations. Romeo's code evaluation is done via Julia built-in functions and text fields are created with GLVisualize.
...
Also, one further complication is, that depending on the platform, function pointers are only available after an \ac{OpenGL} context was created and may only be valid for this context\cite{wgl}.
...
GLAbstraction\cite{GLAbstraction} is the abstraction layer over ModernGL.
...
Besides supplying an abstraction layer over \ac{OpenGL}, it also offers the linear algebra needed for the various 3D transformations and camera code.
...
Creating visualizations with GLAbstraction turns into writing the shader and then combining it with the needed data in the form of OpenGL types.
...
The function with the most methods is \textit{glUniform}, which has around 34 methods for all the different uniform types. 
...
\subsection{GLWindow}
GLWindow\cite{GLWindow} is a lightweight wrapper around GLFW.
...
One cannot create subtrees in advance and then fuse them together at run time.
This led to the design choice, that signals are sampled from a render loop.
This is a sub-optimal approach, as sampling artifacts can occur, and frames are rendered even if the scene does not change.
...
GLVisualize\cite{GLVisualize} implements the visualization functionality.
...
The next design goal was to make this fit for dynamic data, which resulted in relying on as few transformations of the data as possible and directly transferring the data to the GPU.
...
\begin{minipage}{\linewidth}
    ...
    \captionof{figure}[Volumes]{Different visualizations rendered with GLVisualize. As can be seen, every method misses critical features of the volume.}
    \label{fig:volumes}
\end{minipage}
...
Displaying volumes is not a trivial task. 
...
For color picking, two framebuffer render targets need to be created. 
...
So far Romeo\cite{Romeo} just consists of one file with 500 lines of code. 
It just defines some simple text field, a search field, and a visualize and edit window.
The text gets evaluated as Julia code as soon as it changes. Like this, the text field acts like a very simple \ac{REPL}.
