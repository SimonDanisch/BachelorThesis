...
The first API is the \ac{OpenGL} layer. The philosophy is to make the wrapper for native libraries as thin and reusable as possible and an one to one mapping of the underlying library.
...
There is no self reference available in Julia, so the object still needs to be passed to function via the arguments
...
The yellow boxes describe stages which are not directly accessible. They are part of the OpenGL state, which can be set via OpenGL commands.
...
Secondly, the user doe not have to remember or lookup the function name, as long as there is a default visualization for the type he is working with.
