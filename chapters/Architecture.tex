...
To get an idea of the effectiveness of my strategy, usage patterns from Github will be analyzed.
...
Multiple dispatch is basically runtime function overloading. To better understand multiple dispatch, one has to be familiar with Julia's type system.
The type system builds uppon four basic components. Composite types, which are comparable to C structs, parametric composite types, bitstypes, abstract and parametric abstract types.
While the first three are all concrete types, abstract types can be used to build a type hierarchy. Every concrete type can inherit from one abstract type, while abstract types can also inherit from abstract types.
...
\begin{lstlisting}
foo{T}(x::Number, y::Complex{T}) = println("some number: ", x, " some complex Number: ", y) # shorthand function defintion
function foo(x, y::Float32)
    println("overloading foo with a new signature")
end
\end{lstlisting}

This gives Julia a more functional feel, although that you can mimic object oriented programming fairly well.
The functions are also easy to pass around. Functions can be bound to variables and can then be called like normal functions.
One of the most crucial features is the very simple, overheadless C-Interface. 
Thanks to the binary compatibility of LLVM's emited assembly, a C function call to a shared library inside Julia has the same overhead as it would be from inside C\cite{CCALL}. 
This is perfect for dealing with low-level libraries like OpenCL and OpenGL.



\subsection{OpenGL}
OpenGL is a low-level graphics API implemented by the video card vendor via their video driver. 
As such it doesn't offer much abstraction over the actual \ac{GPU}, but instead offers high flexibility and performance.
OpenGL 1.0 was released in 1992 and the current version is 4.5 with 5.0 aka Vulkan just around the corner.
Not all video drivers implement the newest OpenGL standards. 
As a result, one has to decide to which OpenGL version to program against, allowing a trade-off between modernity and platform support.
For Romeo, it was decided to support OpenGL 3.3 as the lowest bound, as it is sufficiently available, while still having most of the modern features.
The features include instance rendering, vertex arrays and modern \ac{GLSL} shader.


\subsection{Reactive}
is an functional event system based on ELM's language principles.

\subsection{GLFW}
GLFW is a cross platform OpenGL context and window creation library written in C.
...
Besides suplying an abstraction layer over OpenGL, it also offers the linear algebra needed for the various 3D transformation and camera code.
