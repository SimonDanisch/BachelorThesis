...
Also, it doesn't allow to rearrange the event tree. In other words if you have to signal branches, there is no way to fuse them together.
Finally, it doesn't reuse memory for the variables. This means, that if you have a signal with a large array, for every lift it will allocate new memory for the array.
So for the event system there had to be a lot of compromises to be made.
