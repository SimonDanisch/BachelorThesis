...
Signals are values, which change over time. They can be transformed via functions, yielding a new signal.
...
First, because it was the only available event system for Julia at that time.
Secondly, it naturally represents change over time, which is a perfect fit for animations.
Due to its forward driven approach, it also gives the ability to render only the values that have changed. 
This gives the system a more greedy behaviour leading to better performance.
...
\ac{OpenGL} is a low-level graphics API implemented by the video card vendor via their video driver. 
As such it doesn't offer much abstraction over the actual \ac{GPU}, but instead offers high flexibility and performance.
\ac{OpenGL} 1.0 was released in 1992 and the current version is 4.5 with 5.0 aka Vulkan just around the corner.
Not all video drivers implement the newest \ac{OpenGL} standards. 
As a result, one has to decide to which \ac{OpenGL} version to program against, allowing a trade-off between modernity and platform support.
