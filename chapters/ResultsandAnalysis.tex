...
This is why we will compare an reactive event graph with its unrolled version.
The unrolled version is basically just the functions in the callback graph executed one after the other. 
This would yield optimal performance and could not be beaten by any event system, which means it is a good baseline.
...
\end{minipage}
\vspace{1em}
This is the first version which manipulates a large array with five chained functions.
As you can see, the performance difference is minimal in this case.
\begin{minipage}{\linewidth}
    \centering
    ...
    \label{fig:reactive1}
\end{minipage}
The other example does not look that favorable. 
The graph has some branches but executes funtions on very small vectors (e.g Vector3).
This seems to be a bad scenario for Reactive. This is bad for the overall performance of Romeo as this is what the Camera code mostly looks like.
