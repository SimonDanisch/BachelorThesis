...
This benchmark is not entirely fair, as Compose3D is just a very rough prototype so far. But I was not able to find any other library with which you could easily create and display interactive 3D graphics in the IJulia or IPython notebook. This benchmark creates a sierpinsky gasket and Compose3D displays it in the IJulia notebook and Romeo displays it natively in a window.
...
It relies on WebGL, which offers only a subset of the OpenGL's functionality. So while the execution speed of OpenGL can be expected to be similar, there are a lot of new techniques missing, which could speed up rendering.
...
You simply pass a signal for every attribute that should change over time.
...
So the usage principle for Romeo is that if you can bring the data into the right format you are fine.
Problems arise, if this can not be done easily.
To be fair, you will have this problem with every kind of visualization API. 
The difference is in the end, how easy it is to transform the data. 
Lets examine an example, where GLVisualize often will not allow to directly call visualize on the data.
There is only a method for visualizing a Mesh, but not for a vertex list plus a face list. If you actually create mesh data, you will often handle the face and vertex list isolated.
So an API that offers a function like \texttt{visualize\_mesh(x::VertexList, y::FaceList)} will be more straightforward for a programmer. Especially, as this is the standard way of displaying a mesh in most scientific plotting packages.
