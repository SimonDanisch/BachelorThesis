...
There are different aspects which can be analyzed. For example, how many function names need to be remembered, how easy they are memorized and how difficult it is to look up unknown functionality.
...
GLAbstraction names: 56

GLVisualize names: 5

GLVisulize has a very simple API, as it basically just has three functions: visualize, visualize_defaults, edit and renderloop.
There is also bounce and loop, which creates signals which bounce and loop between two values.
So for the GLVisualize, only very little function names have to be remembered.