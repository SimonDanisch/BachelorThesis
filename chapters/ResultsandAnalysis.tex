...
This is not an option for performance critical plug-ins though, as Matlab is too slow.
So something like a 3D visualization library should not be written in pure Matlab code. 
For that a mex plug-in is best fitted, which is Matlab's way of letting you write C/C++ plug-ins.
...
For this thesis, I will analyze the main exposed visualization interface, namely GLVisualizes. 
...
There is only a method for visualizing a Mesh, but not for a vertex list plus a face list. If you work with mesh data, you will often handle the face and vertex list in isolation.
...
This is not necessarily an issue. It is quite easy to offer compatibility packages, which defines functions like \texttt{visualize\_mesh(facelist, vertexlist) = visualize(Mesh(facelist, vertexlist))}.
...
The functionality of GLVisualize is easy to explore. There are only three function to be remembered. From there it is easy to find out how they work. First, you can just call visualize on your data and see if the default is already sufficient. If not, you can call visualize_defaults on the data, to find out what kind of defaults there are, which can then be tweaked. As an alternative, one can edit every parameter of the visualization on the spot via the widgets. If that is not enough, a list of the data types that can be visualized can be queried with Julia in builds. In the same way you could also query help for the visualization type.



\subsubsection{GLAbstraction}
GLAbstraction interface is quite a bit more complicated.
There are 57 exported functions in GLAbstraction, with each of them having having between one to five methods.
When possible, OpenGL names were used to keep the naming as easy to remember as possible.