...
But even in the worst case, if the whole architecture is not well fitted for the task, reimplementing large parts would not be difficult as there are only 6641 lines of code which are mostly written in Julia as can be seen table \ref{romeo_loc}.
...
This allows to implement even advanced features like running Julia code directly on the \ac{GPU}\cite{OpenCL}, which may be possible in the next years.
...
While modern Browsers seem to offer context sharing between \ac{WebGL} and \ac{WebCL} (the counter parts for \ac{OpenGL} and \ac{OpenCL}), it must be done from within JavaScript.
