...
The biggest problem with benchmarking the 3D rendering speed is, that there is no library which will allow to exactly reproduce similar circumstances. Additionally, without extensive knowledge of the library, it is difficult to foresee what you actually benchmark. 
As an example of why it is difficult to measure the framerate, consider Vispy. When you enable to measure the framerate, it will show very small numbers, as it only creates a new frame on demand.
On the other  side, Romeo has still a fixed renderloop, which renders as much frames as possible, leading to totally different rendered frames per second. 
This is why it was decided, to use the threshold at which a similar 3D scene is still conceived as enjoyably interactive. This is usually around 25 frames frames per second.
So the benchmark was executed in the way, that the number regulizing the complexity of the 3D scene was increased untill one could not move the camera without stuttering anymore. The threshold is than the benchmark result.

First benchmark is animating a 3D surface plot. The libraries offering this functionality where Vispy and Mayavi.
...
\begin{table}[htbp]
    ...
    \label{table:relativespeedoglw}
\end{table}
Vispy showed some problems, as the camera was never really smooth for the surface example. Also the normals were missing and there was no option to colorize the surface depending on the height.
It was decided to use the threshold of going from a little stutter to unpleasant stutters, making vispy not completely fail.
It would be too much to analyze the exact causes for Romeo being up first. But for vispy it was found out, that the normals where calculated on the CPU. It can be expected that the same goes for Mayavi, just that Mayavi is faster at calculating the normals.
