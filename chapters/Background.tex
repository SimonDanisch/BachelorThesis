...
This is made possible by the compile process which can be described as statically compiled at run-time.
...
So one can write customized floating point type with the same performance and characteristics as the built-in floating point types.
...
In contrast, Matlab, Python, and R need to implement any performance criticial code in another language, which has lead to a programming pattern which is known by the name of vectorization. This pattern has evolved, as the vector operations that are build into the language are much faster than self-written vector operation.
...
It comes with a web-based notebook, which enables you to write formatted documentation together with data, inlined plots and executable program snippets. You can also formulate mathematical formulas in latex, which will get rendered and inlined nicely into an IJulia Notebook.
...
The notebook is completely web based, concentrates on 2D visualizations and interactivity are mostly limited to the programming and not the graphics.
...
It offers a broad range of functionality, including matrix manipulation, plotting of functions and data, the creation of user interfaces and interfacing with a range of languages like C/C++, Java, Fortran and Python. 
...
\end{minipage}
Mayavi is probably one of the biggest, open source library for interactive 3D visualizations in Python.
